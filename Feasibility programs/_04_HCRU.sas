...
%macro rate_tb (seq, lpa_level1, lpa_level2, whrcl1, whrcl2);
	...
	%let groups=overall;
		...
		proc sql;
			create table _11_hosp&z. as select * from derived._07_hosp&ad.&z.&year. where patid in (select distinct patid from _11_sub&z.) and lst_dt>index_date_overall+90;
			create table _11_er&z. as select * from derived._08_er&ad.&z.&year. where patid in (select distinct patid from _11_sub&z.) and lst_dt>index_date_overall+90;
			create table _11_outp&z. as select * from derived._09_outp&ad.&z.&year. where patid in (select distinct patid from _11_sub&z.) and lst_dt>index_date_overall+90;
		quit;
	...
	* Inpatient;
		%do q=1 %to 10;
			...
			proc genmod data=_11_hosp&z. (where= (&vars.>0));
	...
	* ER;
		%do q=1 %to 8;
			...
			ods output ParameterEstimates=_11_est_allpts;
			proc genmod data=_11_er&z. (where= (&vars.>0));
				model &vars.= / offset=ln_py dist=poisson link=log;
			run;
			proc sql;	
				create table _11_rate_cl as
	...
	* outpatient;
		%do q=1 %to 11;
			...
			ods output ParameterEstimates=_11_est_allpts;
			proc genmod data=_11_outp&z. (where= (&vars.>0));
				model &vars.= / offset=ln_py dist=poisson link=log;
			run;
			proc sql;	
				create table _11_rate_cl as
	...
	* Top 20 causes;
		...
		%do q=1 %to 20;
			proc sql;
				...
				from _11_top_cause as a inner join _11_outp&z. as b
				on a.patid=b.patid
				where diag in (select distinct diag from _11_unique_diag where num=&q.);
			...
			ods output ParameterEstimates=_11_est_allpts;
			proc genmod data=_11_top_cause_sub;
...
data derived._11_hcru_rate&year.;
	...
	set _11_hcru_rate2overall1-_11_hcru_rate2overall11;
	...
	set _11_cause_rate2overall1-_11_cause_rate2overall11
...
/*one year*/
ods csv file="/home/dingyig/proj/NOV-27/Feasibility/Output/_11_hcru_rate_one_year.csv";
proc print data=derived._11_hcru_rateone_year;
run;
ods csv close;


ods csv file="/home/dingyig/proj/NOV-27/Feasibility/Output/_11_cause_rate_one_year.csv";
