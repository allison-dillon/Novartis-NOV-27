...
*most recent lab value;
	...
	%create(ldlc_06)
		...
		where year(to_date(b.fst_dt)) >= 2007 and year(to_date(b.fst_dt)) <= 2020 and b.rslt_nbr > 0 and b.rslt_nbr < 9999
...
* save files in SAS;
proc sql;
	create table derived.ldlc_06 as
	select * from heor.ldlc_06_1;
quit;

PROC SQL;
	CREATE TABLE cohort1 as 
	SELECT a.*, b.recent_ldlc, b.r_ldlc_date
	FROM derived._05_demo1b_overall a left join derived.ldlc_06 b
		on a.patid=b.patid
	;
quit;
...
data _07_corr1;
	set 
		_07_cohort1overall1-_07_cohort1overall11 
		_07_cohort1other1-_07_cohort1other11
		_07_cohort1revasc1-_07_cohort1revasc11
		_07_cohort1cvd1-_07_cohort1cvd11
		_07_cohort1anginatia1-_07_cohort1anginatia11
...
data _07_corr_total; 
retain cat1	cat2	seq	cat	Variable N_all	NObs	Mean	StdDev	lpa	Plpa;
set _07_corr1 - _07_corr11;
 run;
...
where index_date_overall is not null;
...
retain cat1	cat2	seq	cat	Variable N_all	NObs	Mean	StdDev	lpa	Plpa;
	set _07_cohort2overall1-_07_cohort2overall11 
		...
		_07_cohort2cvd1-_07_cohort2cvd11
		_07_cohort2anginatia1-_07_cohort2anginatia11
...
* patients with Lp(a) in nmol/dL;

proc sgplot data= 	cohort2;
where index_date_overall is not null;
	scatter x=lpa y=recent_ldlc;
	xaxis label='Lp(a) (nmol/L)';
	yaxis label='LDL-C (mg/dL)';
	reg x=lpa y=recent_ldlc;
run;
