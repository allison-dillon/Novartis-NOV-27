		...
		from dingyig._01_cohort_6&ct. as q
			%do t=1 %to 7;
				...
				left join (select patid, grp, fst_dt, rslt_nbr from dingyig.ldlc_04_lab2 
						where year(to_date(fst_dt)) >= 2007 and to_date(fst_dt) <='2020-06-30 23:59:59.99' and rslt_nbr > 0 and rslt_nbr < 9999 and grp="&lab.")
			...
			%do t=1 %to 7;
				...
				left join (select z.patid, z.grp, z.fst_dt, avg(z.rslt_nbr) as rslt_nbr
							from (select e.*, row_number() over (partition by e.patid, e.grp order by e.gap, e.fst_dt) as rn
								from (select a.patid, grp, b.fst_dt, b.rslt_nbr, abs(datediff(b.fst_dt,a.index_date_&ad.)) as gap
										from dingyig._01_cohort_6&ct. as a inner join (select * from dingyig.ldlc_04_lab2 
																			where year(to_date(fst_dt)) >= 2007 and to_date(fst_dt) <='2020-06-30 23:59:59.99' and rslt_nbr > 0 and rslt_nbr < 9999) as b 
	...
	%create(_12_proc_&ad.&z.&year.)
		...
		from dingyig._12_lab1_&ad.&z.&year. as q
			%do t=1 %to 2;
				...
				left join (select * from dingyig.optum2_01_other_proc  where year(to_date(dt)) >= 2007 and to_date(dt) <='2020-06-30 23:59:59.99' ) as w&t. 
				on q.patid=w&t..patid and w&t..grp="&proc_nm." and q.index_date_&ad. < w&t..dt and w&t..dt<=&years.
			%end;
	/* 			Revascularization */
			left join (select * from dingyig.optum2_01_proc where year(to_date(dt)) >= 2007 and to_date(dt) <='2020-06-30 23:59:59.99'
	...
	%create(_12_demo_tx1_&ad.&z.&year.)
		...
		from dingyig._12_demo_tx_&ad.&z.&year. as q
			%do t=1 %to 16;
				...
				and year(to_date(w&t..dt)) >= 2007 and to_date(w&t..dt) <='2020-06-30 23:59:59.99'
			%end;			
