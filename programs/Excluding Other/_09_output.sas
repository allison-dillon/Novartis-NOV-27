...
%create(_09_outp0)
		...
		from dingyig.raw_00_dod_m as a left join dingyig.raw_00_dod_diag as b
...
%macro outp_ascvd;
	%create(_09_outp_ascvd)
			...
			select distinct a.patid, a.fst_dt
				...
				/*revascularization*/
				,case when b.patid is not null and b.grp in ('revasc') then 1 else 0 end as revasc /*choose all procedure code because cant identify primary procedure*/	

			from dingyig._09_outp_diag_prim as a left join (select * from dingyig.optum2_01_proc where pos in (&outp_pos.) and grp in ('revasc')) as b
...
%create(_09_reh)
		...
		from (select distinct * from dingyig.raw_00_dod_m where pos in (&outp_pos.)) as a
...
%do z=1 %to 2;
%let groups=overall*cvd*revasc*anginatia*other;
		%do zz=1 %to 5;
		%let ad = %scan(&groups., &zz., *);
	proc sql;
		create table _09_outp&ad.&z. as 
		select distinct b.*, a.index_date_&ad., a.eligeff, a.eligend
		from derived._07_primary_&z.&ad. as a 
			inner join (select distinct patid, fst_dt
						%do i=1 %to 11;
							%let grp =%scan(mi*pad*stroke*tia*unsta_angina*sta_angina*other*revasc*cardio*gp*reh, &i., *);
							, max(&grp.) as &grp.
						%end;
						from _09_outp_pre
