...
%create(_07_hosp0)
		...
		from (select *, case when lst_dt is null then fst_dt else lst_dt end as lst_dt1 from dingyig.raw_00_dod_m) as a
	 			...
	 			FROM ( 
						...
						FROM ( 
								...
								COALESCE(LEAD(a.fst_dt,1) OVER (PARTITION BY a.patid ORDER BY a.fst_dt,a.lst_dt), null) as next_start
			 					FROM (select patid, fst_dt, case when lst_dt is null then fst_dt else lst_dt end as lst_dt, pos from dingyig.raw_00_dod_m) as a
...
%macro hosp_diag;
	%create(_07_hosp_er_diag)
			%do i=1 %to 5;
				...
				from dingyig._07_hosp_er_set as a left join dingyig.raw_00_dod_c as b /*use Confinement table*/
			...
			from dingyig._07_hosp_er_set as x
				left join (select a.patid, a.fst_dt, b.diag, case when b.diag_position is null then '99' else b.diag_position end as diag_position
							from dingyig.raw_00_dod_m as a 
								inner join dingyig.raw_00_dod_diag as b /*use Diagnosis table*/
...
%macro hosp_ascvd;
	%create(_07_hosp_ascvd)
			...
			from dingyig._07_hosp_er_diag_prim as a left join (select * from dingyig.optum2_01_proc where pos in (&inp_pos.,&er_pos.) and grp in ('throm','cabg','endar','pci','angio_stent')) as b
...
%create(_07_icu0)
			...
			from dingyig.raw_00_dod_m
			where rvnu_cd in (&icu.) and pos in (&inp_pos.,&er_pos.) /*Revenue code in Medical table*/
			union
			select distinct a.patid, a.fst_dt as dt
			from dingyig.raw_00_dod_m as a
				inner join (select distinct * from dingyig.raw_00_dod_fd where rvnu_cd in (&icu.)) as b /*Revenue code in Facility table*/			
...
/* 			from dingyig.raw_00_dod_c as a */
...
%create(_07_reh0)
		...
		select distinct a.patid, a.fst_dt as dt
		from (select distinct * from dingyig.raw_00_dod_m where pos in (&inp_pos.,&er_pos.)) as a
			inner join (select distinct * from dingyig.raw_00_dod_fd where rvnu_cd in (&rehab_rvnu.) or proc_cd in (&rehab_proc.)) as b
			on a.pat_planid=b.pat_planid and a.clmid=b.clmid and a.clmseq=b.clmseq and a.fst_dt=b.fst_dt
...
%macro enroll;
	...
	format index_date_cvd index_date_noncvd mmddyy10.;
		%do t=1 %to 17;
			%let dt = %scan(lpa_date*eligend*eligeff*index_date_overall*index_date_mi*index_date_stroke*index_date_pad*index_date_unsta_angina*index_date_sta_angina*index_date_tia*index_date_other*index_date_revasc*index_date_throm*index_date_cabg*index_date_endar*index_date_pci*index_date_angio_stent, &t., *);
			format &dt.2 date9.;
			&dt.2=datepart(&dt.);
			drop &dt.;
			rename &dt.2=&dt.;
		%end;
		index_date_cvd= min(datepart(index_date_mi),datepart(index_date_stroke), datepart(index_date_pad));	
		index_date_noncvd=min(datepart(index_date_unsta_angina),datepart(index_date_sta_angina)	,datepart(index_date_tia),datepart(index_date_other),datepart(index_date_revasc),datepart(index_date_throm),datepart(index_date_cabg),datepart(index_date_endar),datepart(index_date_pci),datepart(index_date_angio_stent));

		run;
		
	proc sql;
		create table derived._07_primary_&z.&ad. as 
		select case when index_date_cvd is null then index_date_noncvd end as index_date_noncvd format mmddyy10.
		,a.*
...
%let groups=overall;
	...
	proc sql;
		...
		select distinct b.*, a.patid, a.eligeff, a.eligend, a.index_date_&ad.
		from derived._07_primary_&z.&ad.  as a 
			inner join (select distinct patid, fst_dt, lst_dt
						%do i=1 %to 15;
							%let grp =%scan(mi*pad*stroke*tia*unsta_angina*sta_angina*other*angio_stent*cabg*endar*pci*throm*revasc
											*icu_los*reh_los, &i., *);
							, max(&grp.) as &grp.
						%end;
						from _07_hosp_pre
		...
		on a.patid=b.patid 
			and a.index_date_&ad. le b.fst_dt le (&time.)
...
data _07_hosp2_&ad.&z.; 
	...
	* diagnosis;
	...
	if angio_stent=1 or cabg=1 or endar=1 or pci=1 or throm=1 or revasc=1 then hosp_revasc=1;
...
run;
	...
	proc sql;
		...
		select distinct patid
			...
			%do r=1 %to 21;
				%let cat =%scan(mi*unsta_angina*sta_angina*stroke*tia*pad*angio_stent*cabg*endar*pci*throm
			...
			/* length of stay */
			%do j=1 %to 20;
				%let sub = %scan(mi*unsta_angina*sta_angina*stroke*tia*pad*angio_stent*cabg*endar*pci*throm
								*hosp_ascvd*hosp_mi*hosp_stroke*hosp_pad*hosp_angina*hosp_revasc*hosp_other*reh_cvd*icu, &j., *);
