...
/* PROC SQL; DROP TABLE HEOR._04_cohort_setup1 ; QUIT; */
/* PROC SQL; */
/* 	CREATE TABLE HEOR._04_cohort_setup1 AS  */
/* 	SELECT PUT(patid, 20.) AS patid , index_date_overall, eligeff, eligend FROM DERIVED._07_primary_2overall; */
/* QUIT; */

%create(_04_cohort_setup2)
	select CAST(patid as BIGINT) as patid, index_date_overall, eligeff, eligend
	FROM dingyig._04_cohort_setup1
%create(_04_cohort_setup2);
		...
		%do t=1 %to 16;
			...
			from (select * 
					...
					where loinc_cd in (&&&lab.) 
						...
						%end;	
			) as a inner join dingyig._04_cohort_setup2 as b
			on a.patid=b.patid
			where to_date(fst_dt) <= '2020-06-30 23:59:59.99'
...
%macro comor;
	%create(ldlc_04_comor)
		%do i=1 %to 25;
			...
			and to_date(fst_dt) <= '2020-06-30 23:59:59.99' 
...
%create(ldlc_04_drugs)
			...
			where year(to_date(a.fill_dt)) >= 2007
				and to_date(a.fill_dt) <='2020-06-30 23:59:59.99'
				and a.patid in (select patid from dingyig._04_cohort_setup)
%create(ldlc_04_drugs);
